...
public class BodyArmor : Ammunition
{
    public BodyArmor(string codeName)
    {
        AmmunitionType = "body_armor";
        Code = codeName;

        // Charger les données depuis la base de données SQLite
        using GameDbContext context = new();
        ArmorData armorData = context.Armors
            .FirstOrDefault(a => a.Code == codeName && a.AmmunitionType == "body_armor") ?? throw new InvalidOperationException($"Body armor '{codeName}' not found in database");
